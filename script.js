...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const GITHUB_API_URL = 'https://api.github.com/repos/vydixclothing-cell/game/contents/games.js';

    let games = [...defaultGames];

    async function fetchLiveGames() {
        try {
            // Fetch via GitHub API for instant updates
            ...
            if (arrayMatch) {
                ...
                try {
                    ...
                    try {
                        ...
                        console.error('Parsing failed:', evalErr);
                    }
                }
                console.log('Live games loaded from GitHub API');
                updateCounts();
                renderGames();
            }
        } catch (error) {
            console.error('Failed to fetch live games from API, using default:', error);
            updateCounts();
            renderGames();
        }
    }
    ...
    const renderGames = () => {
        ...
        if (filteredGames.length === 0) {
            ...
            filteredGames.forEach(game => {
                ...
                gameCard.innerHTML = `
                    <a href="game-details.html?game=${slug}" class="game-card-link" style="text-decoration: none; color: inherit;">
    ...
    const openGame = (game) => {
        const slug = game.name.toLowerCase().replace(/[^a-z0-9]/g, '-');
        window.location.href = `game-details.html?game=${slug}`;
    };
